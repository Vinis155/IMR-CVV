...
def main():
    ...
    while running:
       ...
       # Leitura do teclado para controle independente das rodas (CÓDIGO NOVO)
        keys = pygame.key.get_pressed()
        
        v_left_cmd = 0.0
        v_right_cmd = 0.0
        
        # Controle da roda esquerda (W = frente, S = trás)
        if keys[pygame.K_w]:
            v_left_cmd += 120.0
        if keys[pygame.K_s]:
            v_left_cmd -= 120.0
            
        # Controle da roda direita (I = frente, K = trás)
        if keys[pygame.K_i]:
            v_right_cmd += 120.0
        if keys[pygame.K_k]:
            v_right_cmd -= 120.0

        # Passa a usar o método de velocidades das rodas
        robot.set_wheel_velocities(v_left_cmd, v_right_cmd)
        robot.update(dt)
